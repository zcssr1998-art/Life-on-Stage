...
const files=['v3-core.js','ev1.js','ev2.js','ev3.js','ev4.js','ev5.js','ev6.js','ev7.js','ev8.js','v4-data.js','v5-data.js','v5-engine.js','v5-patch.js','v5.2-hotfix.js','v5.3-hotfix.js','v5.4-data.js','v5.4-hotfix.js'];
...
for(let i=0;i<5000;i++){
  ...
  for(const t of d.traits){
    totalSlots++;
    if(['S','SS','SSS','P'].includes(t.rarity))highSlots++;
    if(t.rarity==='B')bSlots++;
  }
}
...
// 3) Natural full-game runs: state must remain valid around mortality/family/economy systems.
...
const naturalMedian=quantile(naturalAges,.5),natural90plus=naturalAges.filter(x=>x>=90).length/naturalAges.length;
if(naturalMedian>86)throw new Error(`natural lifespan still too long: median ${naturalMedian}`);
if(natural90plus>.38)throw new Error(`too many natural lives reach 90+: ${natural90plus}`);

// 4) Isolated mortality curve: standard 60-health / 50-risk person should center near late 70s/early 80s, with 90+ a minority.
...
for(let i=0;i<5000;i++){
  ...
  while(A.p.alive&&A.p.age<113){if(!A.deathCheck())A.p.age++}
...
if(lifeMedian<73||lifeMedian>85)throw new Error(`mortality median out of target: ${lifeMedian}`);
if(life90<.08||life90>.32)throw new Error(`90+ mortality tail out of target: ${life90}`);
if(life100>.08)throw new Error(`100+ still too common: ${life100}`);

// 5) Wealth model: an ordinary non-investing engineer should NOT automatically become multi-millionaire just by surviving.
...
const ordinary=[];
for(let i=0;i<700;i++){
  A.p=economyPerson(false);
  for(let y=0;y<45;y++){A.autoIncome();A.p.age++}
  ordinary.push(A.p.wealth);
}
...
// Investor route should have materially wider tails than the ordinary route.
const investors=[];
for(let i=0;i<700;i++){
  A.p=economyPerson(true);A.p.wealth=500000;
  for(let y=0;y<35;y++){A.autoIncome();A.p.age++}
  investors.push(A.p.wealth);
}
...
for(let i=0;i<180;i++){
  ...
  A.buildYear();const before=A.p.age;clickOnce();
...
fresh();
const oldRender=A.render;A.render=()=>{throw new Error('synthetic ending UI failure')};let deathThrew=false;
...
console.log(`V5.4 smoke passed | traits=${L.TRAITS.length} | prism=${(prismRate*100).toFixed(2)}% hands | high-tier=${(highRate*100).toFixed(1)}% slots | B=${(bRate*100).toFixed(1)}% | natural median age=${naturalMedian}, 90+=${(natural90plus*100).toFixed(1)}% | synthetic median=${lifeMedian}, 90+=${(life90*100).toFixed(1)}%, 100+=${(life100*100).toFixed(1)}% | ordinary wealth median=${Math.round(ordinaryMedian)}, 1m+=${(ordinaryMillion*100).toFixed(1)}% | investor p90-p10=${Math.round(invSpread)}`);
