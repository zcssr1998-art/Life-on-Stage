...
async function uiMetrics(page){
  return page.evaluate(()=>{
    ...
    const kids=[...screen.children].filter(el=>{const r=el.getBoundingClientRect(),s=getComputedStyle(el);return s.display!=='none'&&r.height>0}).map(el=>{const r=el.getBoundingClientRect(),s=getComputedStyle(el);return{c:el.className,top:r.top,bottom:r.bottom,height:r.height,gridRow:s.gridRow,align:s.alignSelf,marginTop:s.marginTop,marginBottom:s.marginBottom}}).sort((a,b)=>a.top-b.top);
    ...
    const latest=document.querySelector('.v55-latest')?.getBoundingClientRect();
    const latestHead=document.querySelector('.v55-latest-head')?.getBoundingClientRect();
    const latestBody=document.querySelector('.v55-story-list,.v55-latest.empty>p')?.getBoundingClientRect();
    return {
      ...
      gridTemplateRows:screenStyle.gridTemplateRows,rowGap:screenStyle.rowGap,
...
test('desktop V6.1 layout has no screenshot-scale empty bands',async({browser})=>{
  ...
  try{
    ...
    expect(m.maxGap).toBeLessThan(24);expect(m.blocked).toBe(0);expect(m.overflow).toBeLessThanOrEqual(5);expect(m.actionInnerGap).toBeLessThan(12);
    const latestHeight=await page.locator('.v55-latest').evaluate(el=>el.getBoundingClientRect().height);expect(latestHeight).toBeLessThan(190);
    const worldHeight=await page.locator('.v6-world').evaluate(el=>el.getBoundingClientRect().height);expect(worldHeight).toBeLessThan(90);
...
test('mobile WebKit survives long sessions, black-box stats, singularity and rewind', async ({ page }) => {
  ...
  const pool=await page.evaluate(()=>{const counts={};for(const t of window.LIFE.TRAITS)counts[t.rarity]=(counts[t.rarity]||0)+1;return{total:window.LIFE.TRAITS.length,counts,micro:window.LIFE.MICRO_EVENTS?.length||0}});
  ...
  await expect(page.locator('.start-traits .trait-chip')).toHaveCount(5);await page.locator('#rerollBtn').click();await page.locator('#startBtn').click();await page.evaluate(()=>{window.APP.deathCheck=()=>false});
  await expect(page.locator('.action-card[data-i]')).toHaveCount(5);await expect(page.locator('.v55-stat')).toHaveCount(10);await expect(page.locator('.v55-top-trait')).toHaveCount(5);await expect(page.locator('.v55-spouse')).toHaveCount(0);await expect(page.locator('.v61-fogbar')).toHaveCount(10);
  const visibleStatWords=await page.locator('.v61-fog-stat .v55-stat-head b').allTextContents();if(visibleStatWords.some(x=>/\d/.test(x)))throw new Error(`exact stat leaked into UI: ${visibleStatWords.join('|')}`);
  ...
  const storyCount=await page.locator('.v55-story').count();expect(storyCount).toBeGreaterThanOrEqual(1);expect(storyCount).toBeLessThanOrEqual(3);const storyText=await page.locator('.v55-story p').first().textContent();expect((storyText||'').length).toBeGreaterThan(18);
  ...
  await page.evaluate(()=>{window.APP.spouse();window.APP.render()});await expect(page.locator('.v55-spouse')).toBeVisible();await expect(page.locator('.v55-spouse')).toContainText('当前伴侣');await page.locator('.nav-btn[data-view="info"]').click();await expect(page.locator('.full-history')).toBeVisible();await expect(page.locator('.stats-grid')).toHaveCount(0);await page.locator('.nav-btn[data-view="game"]').click();
