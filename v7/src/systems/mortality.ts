...
export class MortalitySystem{
  ...
  check(){const s=this.store.get();if(!s.alive)return true;if(s.stats.health<=0){this.store.dispatch({type:'DIE',reason:'健康崩溃'});return true}const age=s.age;if(age>=110){this.store.dispatch({type:'DIE',reason:'自然衰老'});return true}
    ...
    const mods=aggregateBuildMods(s),fragility=.75+((s.seed%997)/996)*.90;const healthMul=clamp(.78,2.45,1+(55-s.stats.health)*.020);const riskMul=clamp(.88,1.72,1+(s.stats.risk-50)*.009);const luckMul=clamp(.91,1.10,1-(s.stats.luck-50)*.0022);const traitDeath=clamp(.45,1.15,mods.death??1);const guard=clamp(.60,1,1-(mods.badGuard??0)*.35);const chance=clamp(0,.97,base*fragility*healthMul*riskMul*luckMul*traitDeath*guard);
