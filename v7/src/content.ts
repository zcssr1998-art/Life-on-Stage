import type {ActionChoice,BaseContent,EffectOp,GameState,LegacyEvent,LegacyOption,Outcome,Requirement,StatKey} from './types.js';
...
export const requirement=(r:any={}):Requirement=>({
  statsMin:r.stats??r.statsMin,statsMax:r.statsMax,tagsAll:r.tagsAll,tagsAny:r.tagsAny,noTags:r.noTags,
  wealthMin:r.wealthMin,wealthMax:r.wealthMax,careerLevelMin:r.professionMin
});
export const passes=(s:GameState,r:Requirement={})=>{
  ...
  if(r.hasRelationship!=null&&((s.relationships.some(x=>x.alive))!==r.hasRelationship))return false;return true;
};
...
const optionOutcomes=(o:LegacyOption):Outcome[]=>{
  if(Array.isArray(o.results)&&o.results.length)return o.results.map((z:any)=>({weight:z.weight??1,text:z.text??o.text??o.label,tone:z.good?'good':undefined,ops:[...effectsOps(z.effects),...(z.addTags??[]).map((tag:string)=>({type:'tag',tag} as EffectOp)),...specialOps(z.special)]}));
...
export const eventToChoice=(e:LegacyEvent,s:GameState,rng:RNG):ActionChoice|null=>{
  const opts=(e.options??[]).filter(o=>passes(s,requirement(o.requires)));if(!opts.length)return null;const o=rng.pick(opts);
  return{id:`${e.id}:${o.label}`,sourceEvent:e.id,category:e.category,title:e.title,desc:o.label,hint:o.hint,rare:!!e.hidden,requires:requirement(o.requires),outcomes:optionOutcomes(o),meta:{repeatable:e.repeatable,cooldown:e.cooldown,weight:e.weight??1}};
};
