...
globalThis.window=globalThis;globalThis.addEventListener=()=>{};globalThis.requestAnimationFrame=f=>f();globalThis.document={getElementById:()=>null,querySelector:()=>null,querySelectorAll:()=>[],documentElement:{dataset:{},style:{setProperty(){}}}};globalThis.navigator={};globalThis.location={href:'https://example.test/'};globalThis.confirm=()=>true;globalThis.innerWidth=390;globalThis.innerHeight=844;globalThis.screen={width:390,height:844};
class StorageMock{constructor(){this.m=new Map()}getItem(k){return this.m.has(k)?this.m.get(k):null}setItem(k,v){this.m.set(k,String(v))}removeItem(k){this.m.delete(k)}clear(){this.m.clear()}}globalThis.localStorage=new StorageMock();
